...
export class Torre extends Peca{
    ...
    public imagem: string | null | undefined = this.Cor === Cor.Branca?
     ...
     movimentosPossiveis(posicao:Posicao): boolean {
       ...
       if(this.posicao?.linha!<posicao.linha){
            const p:Posicao = new Posicao(this.posicao?.linha!+1,this.posicao?.coluna!);
        
            while(p.linha<posicao.linha){
                if(this.tabuleiro.Mostrapeca(undefined,undefined,p)!==null) return false;
 ...
 //Direita
   if(this.posicao?.coluna!<posicao.coluna){
    const p:Posicao = new Posicao(this.posicao?.linha!,this.posicao?.coluna!+1);
